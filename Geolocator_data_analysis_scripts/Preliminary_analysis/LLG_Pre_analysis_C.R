...
thresholdOverLight(lig, threshold, span =c(28000, 31000))
...
for (k in 1:3) {
  fit <- estelleMetropolis(model, x.proposal, z.proposal, x0 = chainLast(fit$x), 
                           z0 = chainLast(fit$z), iters = 500, thin = 20)
...
#load(file = paste0(dir,"/Pre_analysis_C_SGAT_estelle_summary.csv"))
#load(file = paste0(dir,"/Pre_analysis_C_SGAT_estelle_fit.R"))
...
lines(sm[,"Lon.50%"], sm[,"Lat.50%"], 
      col = ifelse(sm$Time1 > fall.equi - days(21) & sm$Time1 < fall.equi + days(21), adjustcolor("blue", alpha.f = 0.6), adjustcolor("firebrick", alpha.f = 0.6)),
      type = "o", pch = 16)
...
plot(sm[(date(sm$Time1) > "2013-09-01" & date(sm$Time1) < "2013-11-01"),]$Time1, sm[(date(sm$Time1) > "2013-09-01" & date(sm$Time1) < "2013-11-01"),]$"Lon.50%", ylab = "Longitude", xlab = "", yaxt = "n", type = "n", ylim = c(min(sm$Lon.mean) - 10, max(sm$Lon.mean) + 10))
axis(2, las = 2)
...
cL <- changeLight(twl=geo_twl, quantile=0.85, summary = F, days = 7, plot = T)
...
points(dtx0, pch=19, col= ifelse(stationary == F, "cornflowerblue", "orange"), type = "o")
points(lon.calib, lat.calib, pch = 16, cex = 2.5, col = "firebrick")
box()
...
#save(sm, file = paste0(dir,"/Pre_analysis_C_SGAT_GroupedThreshold_summary.csv"))
#save(fit, file = paste0(dir,"/Pre_analysis_C_SGAT_GroupedThreshold_fit.R"))
...
# empty raster of the extent
r <- raster(nrows = 2 * diff(ylim), ncols = 2 * diff(xlim), xmn = xlim[1]-5,
            xmx = xlim[2]+5, ymn = ylim[1]-30, ymx = ylim[2]+5, crs = proj4string(wrld_simpl))
