...
#                        gr.Device = "default", # MacOS version (and windows)
#                        dark.min = 120)
# 
...
geo_twl_adjusted <- export2GeoLight(twl_adjusted)
...
grouped[twl_adjusted.rev$Site>0] <- TRUE 
...
#fixedx[1] <- TRUE # We don't fix the last location estimate because the track is incomplete  

# We fixed the first and last location because the geolocator stopped working close to it's breeding site
fixedx[1] <- TRUE
...
par(mfrow=c(1,1))
colours <- c("black",colorRampPalette(c("blue","yellow","red"))(max(twl_adjusted.rev$Site)))
data(wrld_simpl)
...
# However, the Light transition data is unclear and the stopover would be short (2-3 days)

# Estimate timing of departure and arrival from the breeding and nonbreeding grounds ############################################################
dep.br <- "2018-08-26"
arr.br <- NA# couldn't estimate return date due to short day length 
