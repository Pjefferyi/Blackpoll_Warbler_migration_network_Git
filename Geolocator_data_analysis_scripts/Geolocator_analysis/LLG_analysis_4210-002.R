...
log.prior <- function(p) {
  ...
  ifelse(is.na(f), log(1), log(1.5)) # if f indicates the distribution of the blackpoll warbler 
...
sm <- sm %>% mutate(period= case_when(StartTime < anytime("2016-10-15 21:56:44", asUTC = T, tz = "GMT")  ~ "Post-breeding migration",
                                      StartTime >= anytime("2016-10-15 21:56:44", asUTC = T, tz = "GMT") & StartTime < anytime("2017-04-28 22:31:04" , asUTC = T, tz = "GMT") ~ "Non-breeding period",
                                      StartTime > anytime("2017-04-28 22:31:04", asUTC = T, tz = "GMT") ~ "Pre-breeding migration"))
