...
deploy.start <- anytime("2019-06-18", asUTC = T, tz = "GMT")
...
# Initial Path #################################################################
path <- thresholdPath(twl$Twilight, twl$Rise, zenith = zeniths_med, tol=0.15)
...
# Fit the model
fit <- estelleMetropolis(model, x.proposal, z.proposal, iters = niters, thin = nthin, chain = chains)
...
fit <- estelleMetropolis(model, x.proposal, z.proposal, x0 = chainLast(fit$x),
                         z0 = chainLast(fit$z), iters = niters, thin = nthin, chain = 1)
...
sm <- sm %>% mutate(period= case_when(StartTime < anytime("2019-10-18 10:27:53", asUTC = T, tz = "GMT")  ~ "Post-breeding migration",
                                      StartTime >= anytime("2019-10-18 10:27:53", , asUTC = T, tz = "GMT") ~ "Non-breeding period"))
...
# A stopover over the Caribbean was identified between approximately 2019-10-03 and 2019-10-19
