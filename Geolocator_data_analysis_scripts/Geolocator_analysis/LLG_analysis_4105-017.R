...
tm.calib1 <- as.POSIXct(c("2016-07-05", "2016-08-20"), tz = "UTC")
tm.calib2 <- as.POSIXct(c("2017-06-10", "2017-06-20"), tz = "UTC")
...
path <- thresholdPath(twl$Twilight, twl$Rise, zenith = zeniths_med, tol= 0.00)
...
# Initial Path #################################################################
path <- thresholdPath(twl$Twilight, twl$Rise, zenith = zenith, tol=0.09)
...
sm <- sm %>% mutate(period= case_when(StartTime < anytime("2016-10-23 22:13:55", asUTC = T, tz = "GMT")  ~ "Post-breeding migration",
                                      StartTime >= anytime(" 2016-10-23 22:13:55", asUTC = T, tz = "GMT") & StartTime < anytime("2017-04-23 22:35:02" , asUTC = T, tz = "GMT") ~ "Non-breeding period",
                                      StartTime > anytime("2017-04-23 22:35:02", asUTC = T, tz = "GMT") ~ "Pre-breeding migration"))
