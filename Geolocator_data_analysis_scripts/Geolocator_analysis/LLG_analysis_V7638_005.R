...
spring.equi <- anytime("2019-03-20", asUTC = T, tz = "GMT")
...
# This geolocator has no visible time shift 
...
tm.calib1 <- as.POSIXct(c("2018-07-25", "2018-08-20"), tz = "UTC")
...
zenith_twl_med <- data.frame(Date = twl$Twilight) %>%
  mutate(zenith = case_when(Date < anytime(arr.nbr) ~ zenith,
                            Date > anytime(arr.nbr) & Date < anytime(dep.nbr) ~ zenith_sd, # added so latitude statys constant in nonbreeding period 
...
sm <- sm %>% mutate(period= case_when(StartTime < anytime("2018-11-10 22:21:05", asUTC = T, tz = "GMT")  ~ "Post-breeding migration",
                                      StartTime >= anytime("2018-11-10 22:21:05", asUTC = T, tz = "GMT") & StartTime < anytime("2019-04-20 22:29:14", asUTC = T, tz = "GMT") ~ "Non-breeding period",
                                      StartTime > anytime("2019-04-20 22:29:14", asUTC = T, tz = "GMT") ~ "Pre-breeding migration"))
