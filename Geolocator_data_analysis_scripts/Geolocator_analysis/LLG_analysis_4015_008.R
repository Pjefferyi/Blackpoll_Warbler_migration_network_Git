...
tm.calib2 <- as.POSIXct(c("2017-06-08", "2017-06-22"), tz = "UTC")
...
zenith_twl_zero <- data.frame(Date = twl$Twilight) %>%
  mutate(zenith = case_when(Date < anytime(arr.nbr) ~ zenith0 + 2, 
                            ...
                            Date > anytime(dep.nbr) ~ zenith0 + 2))
...
points(path$x[1:500,], pch=19, col="cornflowerblue", type = "o")
...
sm <- sm %>% mutate(period= case_when(StartTime < anytime("2016-10-22 22:10:16 ", asUTC = T, tz = "GMT")  ~ "Post-breeding migration",
                                      StartTime >= anytime("2016-10-22 22:10:16 ", asUTC = T, tz = "GMT") & StartTime < anytime("2017-04-24 10:28:11" , asUTC = T, tz = "GMT") ~ "Non-breeding period",
                                      StartTime > anytime("2017-04-24 10:28:11", asUTC = T, tz = "GMT") ~ "Pre-breeding migration"))
...
geo.ref[(geo.ref$geo.id == geo.id),]$Clock_drift_edits <- FALSE
