...
tm.calib1 <- as.POSIXct(c("2018-07-25", "2018-08-25"), tz = "UTC")
...
zenith_twl_zero <- data.frame(Date = twl$Twilight) %>%
  mutate(zenith = case_when(Date < anytime(arr.nbr) ~ zenith0,
                            Date > anytime(arr.nbr) & Date < anytime(dep.nbr) ~ zenith0_ad + 3.5,
                            Date > anytime(dep.nbr) ~ zenith0_ad + 3.5))
...
plot(lig$Date[lig$Date > anytime(start) & lig$Date < anytime(end)], lig$Light[lig$Date > anytime(start) & lig$Date < anytime(end)], type = "o",
