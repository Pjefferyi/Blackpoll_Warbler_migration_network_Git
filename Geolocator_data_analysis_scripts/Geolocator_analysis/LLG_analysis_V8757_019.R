...
# tag number: V8757-019
# site: British Columbia, Highway 37
...
period <- as.POSIXct(c("2019-07-15", "2019-08-15"), tz = "UTC")
...
tm.calib <- as.POSIXct(c("2019-07-15", "2019-08-28"), tz = "UTC")
...
zenith_twl_zero <- data.frame(Date = twl$Twilight) %>%
  mutate(zenith = case_when(Date < anytime("2019-10-28") ~ zenith0,
                            Date > anytime("2019-10-28") & Date < anytime("2020-04-28") ~ zenith0_ad,
                            Date > anytime("2020-04-28") ~ zenith0))

zeniths0 <- zenith_twl_zero$zenith

zenith_twl_med <- data.frame(Date = twl$Twilight) %>%
  mutate(zenith = case_when(Date < anytime("2019-10-28") ~ zenith,
                            Date > anytime("2019-10-28") & Date < anytime("2020-04-28") ~ zenith_sd,
...
# merge site helps to put sites together that are separated by single outliers.
mS <- mergeSites(twl = geo_twl, site = cL$site, degElevation = 86-zeniths0[1: length(zeniths0) -1], distThreshold = 1000)
...
par(mfrow=c(3,1))
plot(lig$Date[lig$Date < "2019-10-10" & lig$Date > "2019-09-20"], lig$Light[lig$Date < "2019-10-10" & lig$Date > "2019-09-20"], type = "o")
plot(twl$Twilight[twl$Twilight< "2019-10-10" & twl$Twilight > "2019-09-20"], x0[,1][twl$Twilight< "2019-10-10" & twl$Twilight > "2019-09-20"])
plot(twl$Twilight[twl$Twilight< "2019-10-10"& twl$Twilight > "2019-09-20"], x0[,2][twl$Twilight< "2019-10-10" & twl$Twilight > "2019-09-20"])
