...
cL <- changeLight(twl=geo_twl, quantile= 0.67, summary = F, days = 2, plot = T)
...
fit <- estelleMetropolis(model, x.proposal, z.proposal, x0 = chainLast(fit$x),
                         z0 = chainLast(fit$z), iters = 300, thin = 20, chain = 1)
...
#second flight
f2.start <- "2018-10-21 00:00"
f2.end <- "2012-18-21 12:00"
