...
# Alternative calibration not possible here as only a short period of the nonbreeding period is included in the track
...
# We only fix the deployment location because this geolocator track is incomplete
...
fixedx[1] <- TRUE #We only fix the first location because this geolocator track is incomplete 
...
sm <- sm %>% mutate(period= case_when(StartTime < anytime(" 2016-10-21 22:31:15", asUTC = T, tz = "GMT")  ~ "Post-breeding migration",
                                      StartTime >= anytime(" 2016-10-21 22:31:15", asUTC = T, tz = "GMT") ~ "Non-breeding period"))
