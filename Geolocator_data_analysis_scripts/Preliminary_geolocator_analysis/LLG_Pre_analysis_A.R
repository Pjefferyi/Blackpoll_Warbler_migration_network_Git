...
lig <- lig[(is.na(lig$comments) == TRUE),]  %>%
  ...
  # rename columns  
  dplyr::rename(c(Date = timestamp, Light = gls.light.level)) %>%
  #convert to as.POXIct
...
lig$Date <- anytime(lig$Date, asUTC = T, tz = "UTC")
...
#write.csv(twl_in, paste0(dir,"/Pre_analysis_A_twl_times_initial.csv"))
twl_in <- read.csv(paste0(dir,"/Pre_analysis_A_twl_times_initial.csv"))
...
period <- as.POSIXct(c("2013-07-03", "2013-08-03"), tz = "UTC")
...
tm.calib <- as.POSIXct(c(deploy.start + days(5), deploy.start + days(45)), tz = "UTC")
...
#save(sm, file = paste0(dir,"/Pre_analysis_A_SGAT_estelle_summary.csv"))
#save(fit, file = paste0(dir,"/Pre_analysis_A_SGAT_estelle_fit.R"))

#load the output of the estelle model 
#load(paste0(dir,"/Pre_analysis_A_SGAT_estelle_summary.csv"))
#load(paste0(dir,"/Pre_analysis_A_SGAT_estelle_fit.R"))
...
mS <- mergeSites(twl = geo_twl, site = cL$site, degElevation = 90-zenith_sd, distThreshold = 1000)
...
#save(sm, file = paste0(dir,"/Pre_analysis_A_SGAT_GroupedThreshold_summary.csv"))
#save(fit, file = paste0(dir,"/Pre_analysis_A_SGAT_GroupedThreshold_fit.R"))
...
twlexp <- twGeos2TAGS(raw = lig[, c("Date", "Light")], twl = twl,
                      threshold = 1.5,
                      filename = paste0(dir, "/A_TAGS_data.csv"))

tags <- get.tags.data(paste0(dir, "/A_TAGS_data.csv"))
...
abline(v=as.POSIXct("2013-09-02"), col = "green") # end of first calibration period
abline(v=as.POSIXct("2014-05-20"), col = "green") # start of the second calibration period

Calibration.periods<-data.frame(
  calibration.start=as.POSIXct(c(NA,"2014-05-20")),
  calibration.stop=as.POSIXct(c("2013-09-02",NA)),
...
Grid <- make.grid(left=lon.calib -20, bottom=lat.calib-60, right=lon.calib+50, top= lat.calib + 10,
...
#save(all.in, file = paste0(dir, "/A_FlightRCalib.RData"))

#run twilight filter ###########################################################

#Load results 
load(paste0(dir, "/A_FlightRCalib.RData"))
...
Result$Results$Quantiles[c(348,479),]$time

# The new calibration period ranges between "2013-12-05 10:32:57 GMT" "2014-02-08 23:01:07 GMT"
NB.lat =  8.1 
NB.lon =  -69.09091
...
Calibration.periods<-data.frame(
  calibration.start=as.POSIXct(c("2013-12-05 10:32:57 GMT")),
  calibration.stop=as.POSIXct(c("2014-02-08 23:01:07 GMT")),
  lon=NB.lat, lat= NB.lon) 
# use c() also for the geographic coordinates, 
# if you have more than one calibration location
# (e. g.,  lon=c(5.43, 6.00), lat=c(52.93,52.94))
print(Calibration.periods)
