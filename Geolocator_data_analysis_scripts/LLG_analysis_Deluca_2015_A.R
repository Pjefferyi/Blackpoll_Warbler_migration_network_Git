...
#get data for individual A
Adata <- bpdata2015[(bpdata2015$individual.local.identifier == "A"),]
...
Aldata <- rename(Adata, Date = timestamp, Light = gls.light.level) %>%
...
with(Aldata[1000:10000,], plot(Date, Light, type = "o"))
abline(h=threshold, col="orange", lty = 2, lwd = 2)

#another plot 
offset = 15 # adjusts the y-axis to put night (dark shades) in the middle

lightImage( tagdata = Aldata,
            offset = offset,     
            zlim = c(0, 20))

tsimageDeploymentLines(Aldata$Date, lon = lon.calib, lat = lat.calib,
                       offset = offset, lwd = 3, col = adjustcolor("orange", alpha.f = 0.5))

savings.t <- as.POSIXct(c("2013-11-06", "2014-03-12"), tz = "UTC")
...
twlA2015 <- twilightEdit(twilights = twl,
                          offset = offset,
...
tsimagePoints(twlA2015$Twilight, offset = offset, pch = 16, cex = 1.2,
...
tsimageDeploymentLines(twlA2015$Twilight, lon.calib, lat.calib, offset, lwd = 2, col = "orange")

tm.calib <- as.POSIXct(c("2013-06-13", "2013-09-01"), tz = "UTC")
abline(v = tm.calib, lwd = 2, lty = 2, col = "orange")

d_calib <- subset(twlA2015, Twilight>=tm.calib[1] & Twilight<=tm.calib[2])
...
Calibration.periods<-data.frame(
  ...
  calibration.stop=as.POSIXct(c('2013-09-01')),
